...
export default function ConversationPage() {
  ...
  if (loading) return <div className="echo-dialogue"><p role="status">로그인 상태를 확인하고 있어요.</p></div>;
  if (!user) return <section className="echo-dialogue"><p className="echo-eyebrow">ECHO · 내 이야기</p><h1>다음에 와도,<br />이야기가 이어지도록.</h1><p className="echo-lead">로그인하면 내 이야기와 고친 설명을 내 계정에 보관할 수 있어요.</p><Link className="echo-primary" to="/login" state={{ from: `/doit/conversation${search.get('from') === 'journey' ? '?from=journey' : ''}` }}>로그인하고 이야기하기</Link><Link className="echo-secondary" to="/doit/start-journey">내 프로필 준비하기</Link></section>;
  ...
  if (purposeState.kind === 'loading') return <div className="echo-dialogue"><p role="status">내 선택을 불러오고 있어요.</p></div>;
