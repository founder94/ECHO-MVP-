...
const LINE_MAX = 300;
const SAVE_ERROR = '선택을 저장하지 못했어요. 연결을 확인하고 다시 눌러 주세요.';
...
export default function ConversationOpening({ userId, onDone }: Props) {
  ...
  return <section className="echo-dialogue echo-opening" aria-busy={saving}>
    ...
    <p className="echo-eyebrow">ECHO의 첫 질문</p>
    <h1>어떤 만남을<br />원하세요?</h1>
    <p className="echo-lead">하나를 고르고, 원하면 한 줄만 덧붙여 주세요. 그 말에서 대화가 시작돼요.</p>
    ...
    {selected && <form className="echo-composer echo-opening-line" onSubmit={event => { event.preventDefault(); void submit(); }}>
      <label htmlFor="echo-opening-line">한 줄만 더 적어도 좋아요 (선택)</label>
    ...
    <p className="echo-fine">선택은 내 계정에만 저장돼요. 언제든 대화에서 바꿀 수 있어요.</p>
  </section>;
}
