...
test('first screen renders without a blank lazy-loading gap', async () => {
  ...
  assert.match(fallback, /화면을 준비하고 있어요\./);

  // 4) 가짜 진행률은 만들지 않는다(진행 막대·숫자 % 표시가 코드에 없어야 한다).
  assert.doesNotMatch(fallback, /role="progressbar"|aria-valuenow|<progress/i);
  assert.doesNotMatch(fallback, /\d+\s*%/);
