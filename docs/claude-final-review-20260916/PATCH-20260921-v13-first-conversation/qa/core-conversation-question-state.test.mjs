...
function componentHarness(overrides = {}, { followup = true, server = true, pending = null, transport = null } = {}) {
  const slots = [];
  ...
  function render() { cursor = 0; dirty = false; tree = exports.default({ userId: 'owner' }); }
...
test('generation failure preserves the original without inventing a question', async () => {
  ...
  assert.deepEqual(h.questions(), []);
  assert.match(h.content(), /내 원문 a/);
  assert.match(h.content(), /아직 결과를 확인하지 못했어요/);
});
...
test('pending direct explanation reopens the editor with its text after reload and saves without a second rejection', async () => {
  ...
  assert.match(h.content(), /이전 AI 해석은 제외했어요/);
  h.click('이 설명으로 저장하기 ');
  await h.flush();
  ...
  assert.match(h.content(), /직접 설명해 주신 내용으로 저장했어요/);
  h.unmount();
});
