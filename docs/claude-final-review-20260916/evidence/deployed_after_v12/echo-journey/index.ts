...
interface Ctx { mindText: string; messages: Msg[]; understandings: Und[]; priorSummary?: string }
...
// ── 2026-09-16 행동 분기(Action Router)·되받아치기 방지 ──
...
function cleanReply(raw: string): string {
  ...
  if (!text || text.includes("?") || forbidden(text)) return "";
  return text;
}
...
type BlockReason = "forbidden" | "ack_anchor" | "quality" | "repeat_text" | "repeat_intent" | "anchor_reuse" | "rejected_key" | "rejected_text";
// relaxed(마지막 시도): 의도 반복·중심 표현 재사용 같은 '다양성' 규칙만 풀고, 안전·근거·거절 규칙은 유지한다.
function blockReason(c: Candidate, b: Block, evidenceParts: string[], intentHistory: string[], avoidAnchorReuse = false, relaxed = false): BlockReason | null {
...
function blocked(c: Candidate, b: Block, evidenceParts: string[], intentHistory: string[], avoidAnchorReuse = false, relaxed = false): boolean {
  return blockReason(c, b, evidenceParts, intentHistory, avoidAnchorReuse, relaxed) !== null;
}
...
function renderCandidate(candidate: Candidate, mode: QuestionMode, feedbackKind = journeyFeedbackKind(""), latestUser = ""): string {
  if (mode === "asked") {
    // 사용자가 물은 말에는 먼저 짧게 답하고(reply), 그 다음 질문 하나로 이어간다.
    return `${candidate.reply || ASKED_FALLBACK_REPLY}\n\n${candidate.question}`;
  }
  if (mode === "feedback") {
...
async function genStepQuestion(ai: Ai, ctx: Ctx, status: StepStatus): Promise<string> {
  ...
  const userQuestionNote = asked
    ...
    : "";
  const priorNote = ctx.priorSummary
    ? `\n\n[지난 여정에서 나눈 이야기 요약 — 사용자 사실로 새로 만들지 말고, 같은 주제가 다시 나오면 이어서 반영]\n${ctx.priorSummary}`
    : "";
  ...
  for (let i = 0; i < LIMITS.ATTEMPTS; i++) {
    ...
    for (const c of cands) {
      const reason = blockReason(c, block, evidenceParts, intentHistory, avoidAnchorReuse, relaxed);
      if (!reason) {
        survivor = c;
        break;
      }
    ...
    if (survivor) return renderCandidate(survivor, mode, feedbackKind, latestAnswer);
    blockedAll = blockedAll.concat(cands.map((c) => c.question));
    // 진단 로그: 단계·모드·후보 수·차단 사유 수만. 원문 없음.
    console.error(`[ej] candidates_blocked step=${stepOf(status)} mode=${mode} parsed=${cands.length} attempt=${i + 1} relaxed=${relaxed} reasons=${Object.entries(reasons).map(([k, v]) => `${k}:${v}`).join(",")}`);
  }
...
async function loadPriorSummary(sb: Db, userId: string, excludeConversationId: string): Promise<string> {
  ...
  const prior = (data as Json[]).find((row) => String(row.conversation_id ?? "") !== excludeConversationId);
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (action === "ask") {
      ...
      before.priorSummary = await loadPriorSummary(sb, user.id, conv.id);
