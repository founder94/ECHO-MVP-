...
test('v14.3 처음부터 다시 — 대화 화면 위쪽에도 있고, 누른 자리 옆에 확인 창이 뜬다', async () => {
  ...
  assert.match(c, /onClick=\{\(\) => setRestartArmed\('top'\)\}>처음부터 다시 하기</);
  assert.match(c, /onClick=\{\(\) => setRestartArmed\('bottom'\)\}>처음부터 다시 시작하기</);
...
test('v14.3 앱 홈의 「처음부터 다시 시작하기」는 실제로 다시 시작하는 확인 창을 연다', async () => {
  ...
  assert.match(home, /to="\/doit\/conversation\?restart=1">처음부터 다시 시작하기/);
