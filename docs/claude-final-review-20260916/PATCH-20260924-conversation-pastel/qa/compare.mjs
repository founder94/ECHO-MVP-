...
const OPTS = {
  A: '',
  B: '.echo-dialogue.echo-dialogue--pastel{--echo-pastel-veil:.5!important}',
  C: '.echo-dialogue.echo-dialogue--pastel{--echo-pastel-veil:.86!important}',
  D: `.echo-dialogue.echo-dialogue--pastel :is(h1,.echo-question,.echo-lead,.echo-ack,.echo-topic,.echo-eyebrow,.echo-fine,.echo-original,.echo-original p,.echo-history,.echo-history summary,.echo-steps-count,.echo-steps-count em,.echo-composer label,.echo-composer-footer>span,.echo-dialogue-header>span,.echo-dialogue-header>a,.echo-notice){color:#0c1526!important;-webkit-text-fill-color:#0c1526!important;background-image:none!important}`,
};
for (const [key, css] of Object.entries(OPTS)) {
  const { ctx, page } = await open(390, 844, 'question');
  await page.goto(`${BASE}/doit/conversation`); await page.waitForTimeout(2500);
  if (css) await page.addStyleTag({ content: css });
  await page.addStyleTag({ content: '*,*::before,*::after{animation:none!important}' });
  await page.evaluate(() => window.scrollTo(0, 380)); await page.waitForTimeout(300);
  await page.screenshot({ path: `${DIR}/option-${key}.png` });
  await ctx.close();
}
