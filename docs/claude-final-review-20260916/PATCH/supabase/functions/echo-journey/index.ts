...
function parseReport(raw: string, rejectedTexts: string[]): Report | null {
  ...
  for (const s of o.sections as unknown[]) {
    ...
    sections.push({ heading, body, status: x.status === "confirmed" ? "confirmed" : "candidate" });
...
async function genReport(ai: Ai, ctx: Ctx): Promise<Report> {
  ...
  const system = `${PERSONA} 제공된 [사용자 근거]만 바탕으로 사용자가 '진짜 나'를 이해하도록 돕는 자기이해 리포트를 한국어로 써라. 이전 ECHO 질문·요약은 근거가 아니다. 카피의 주어는 '나/내가'다. JSON 객체로만 출력한다. 형식: {"title":"40자 이내 제목","summary":"2~3문장 요약","sections":[{"heading":"소제목","body":"3~6문장","status":"confirmed 또는 candidate"}],"next_step":"지금 해볼 수 있는 아주 작은 한 걸음 1~2문장"}. sections 는 3~5개. status 규칙: 사용자가 직접 말한 내용만으로 쓴 부분은 "confirmed", 조심스러운 해석이 섞인 부분은 "candidate" 로 표시하고 문장도 '~인 것 같아요'로 쓴다. 반드시 지켜라: 1) 사용자가 직접 설명·정정한 내용을 가장 우선으로 반영한다. 2) 거절한 해석은 표현을 바꿔도 다시 쓰지 않는다. 3) 사용자가 말하지 않은 사람·관계·사건·감정·원인·회피·상처를 만들지 않는다. 4) 진단·치료·점술·성격검사식 단정, 데이팅·궁합 표현을 쓰지 않는다.${priorityNote(ctx)}`;
  for (let i = 0; i < LIMITS.ATTEMPTS; i++) {
    const report = parseReport(await callOpenAI(ai, system, `[사용자 근거]\n${userEvidenceText(ctx)}`, true, REPORT_MAX_TOKENS), rejectedTexts);
