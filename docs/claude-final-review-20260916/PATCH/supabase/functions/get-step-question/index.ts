...
function parseCandidates(raw: string): ParseResult {
  ...
  for (const item of list) {
    ...
    const assumptions = Array.isArray(o.assumptions)
      ...
      : ["schema_missing"];
    candidates.push({ acknowledgement: typeof o.acknowledgement === "string" ? o.acknowledgement.trim().slice(0, 100) : "", question, meaning, keys: cleanKeys(o.keys), anchor, assumptions });
...
function blockReasonFor(c: Candidate, ctx: BlockContext): BlockReason | null {
  ...
  if (repeatsQuestionIntent(c.question, ctx.askedTexts)) return "repeat";
...
async function genSingleQuestion(ai: Ai, instruction: string, userContent: string, ctx?: Context): Promise<string> {
  ...
  for (let attempt = 0; attempt < LIMITS.GENERATION_ATTEMPTS; attempt++) {
    ...
    const raw = await callOpenAI(ai, [{ role: "system", content: `${PERSONA} ${instruction} 친구처럼 바로 앞 말을 짧게 받아준 뒤, 아직 답하지 않은 새로운 정보를 부탁하는 열린 질문 하나를 써라. 사용자의 말을 거의 그대로 옮기고 물음표만 붙이는 되묻기와 예/아니오 확인 질문은 금지한다. 질문 피드백이 있으면 잘못을 짧게 인정하고 더 쉽고 다른 방향으로 묻되 그 피드백을 사용자 마음의 근거로 해석하지 마라. 사용자가 말하지 않은 사람·관계·미래 장면·감정·원인·회피·상처를 만들지 마라. 전체 문장에는 물음표가 하나만 있어야 한다.` }, { role: "user", content: prompt }], false);
...
async function genFollowupQuestion(ai: Ai, ctx: Context): Promise<Candidate> {
  ...
  const system =
    `${PERSONA} 아래 [사용자 근거]만 사실로 사용해서 아직 더 알아가야 할 부분을 묻는 후보 3개를 만들어라. 각 후보는 {"acknowledgement":"anchor를 글자 그대로 포함해 바로 앞 사용자 말을 짧게 받아주는 1문장","question":"새로운 정보를 부탁하는 열린 질문 1개","anchor":"사용자 근거에서 글자 그대로 가져온 표현","assumptions":[],"meaning":"이전 질문과 다른 새 질문 의도","keys":["핵심 의미 명사구 2~5개"]} 형태이고, 전체를 {"candidates":[...]} JSON 객체로만 출력한다. 반드시 지켜라: 1) acknowledgement에는 anchor를 그대로 넣되 question에는 복사하지 않아도 된다. 2) 사용자의 말을 거의 그대로 옮기고 물음표만 붙이는 되묻기, 예/아니오 확인 질문, 이미 답한 내용을 다시 묻는 질문은 금지한다. 3) 사용자가 말하지 않은 사람·관계·미래 장면·감정·원인·회피·상처·행동을 만들지 않는다. 4) 사용자가 거절한 해석과 같은 뜻은 표현을 바꿔도 만들지 않는다. 5) 사용자가 직접 설명·정정한 내용을 가장 먼저 반영한다. 6) 한 번에 한 가지만 묻는다.${priorityNote(ctx)}`;
  ...
  for (let attempt = 0; attempt < LIMITS.GENERATION_ATTEMPTS; attempt++) {
    ...
    const candidates = feedback ? parsed.candidates.map((candidate) => ({ ...candidate, acknowledgement: "" })) : parsed.candidates;
    const result = filterCandidates(candidates, block);
