...
// 대화 화면 어두운 판(Surface) 지금 CSS 실측
const SEL = ['#echo-message', '.echo-secondary', '.echo-restart-pill', '.echo-opening-tile', '.echo-brief', '.echo-restart', '.echo-reactions button', '.echo-question', '.echo-dialogue h1', '.echo-lead', '.echo-original > p:last-child', '.echo-eyebrow', '.echo-steps-count', '.echo-steps-bar', '.echo-steps-bar i', '.echo-fine', '.echo-composer label', '.echo-error'];
const out = {};
for (const scn of ['opening', 'start', 'question', 'error', 'restart']) {
  const { ctx, page } = await open(390, 844, scn);
  await page.goto(`${BASE}/doit/conversation`); await page.waitForTimeout(2500);
  ...
  if (scn === 'restart') { await page.click('.echo-restart-top .echo-restart-pill'); await page.waitForTimeout(500); }
  const r = await page.evaluate((SEL) => Object.fromEntries(SEL.map((sel) => { const el = document.querySelector(sel); if (!el) return [sel, null]; const s = getComputedStyle(el); const ph = el.tagName === 'TEXTAREA' ? getComputedStyle(el, '::placeholder').color : undefined; return [sel, { background: s.backgroundColor, bgImage: s.backgroundImage.slice(0, 50), border: `${s.borderTopWidth} ${s.borderTopColor}`, radius: s.borderRadius, shadow: s.boxShadow.slice(0, 60), backdrop: s.backdropFilter || s.webkitBackdropFilter, opacity: s.opacity, color: s.color, fill: s.webkitTextFillColor, textShadow: s.textShadow, ...(ph ? { placeholder: ph } : {}) }]; })), SEL);
  for (const [k, v] of Object.entries(r)) if (v && !out[k]) out[k] = v;
  await ctx.close();
}
