...
// 전후 캡처: 첫 화면 + 맨 아래(스크롤을 바로 이동하고 멈춘 뒤)
const shotsReport = [];
for (const [w, h] of WIDTHS) for (const scn of ['opening', 'opening-selected', 'question', 'error', 'restart']) for (const bottom of [false, true]) {
  const base = scn === 'opening-selected' ? 'opening' : scn;
  const { ctx, page, errors } = await open(w, h, base);
  await page.goto(`${BASE}/doit/conversation`); await page.waitForTimeout(2500);
  ...
  await page.evaluate((b) => window.scrollTo({ top: b ? document.documentElement.scrollHeight : 0, behavior: 'instant' }), bottom); await page.waitForTimeout(1200);
  const overflow = await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth);
  await page.screenshot({ path: `${DIR}/${w}-${scn}${bottom ? '-bottom' : ''}.png` });
  shotsReport.push({ w, scn, bottom, overflow, errors: errors.length }); await ctx.close();
}
writeFileSync(`${DIR}/shotsReport.json`, JSON.stringify(shotsReport, null, 1));
console.log('캡처', shotsReport.length, '가로 넘침', shotsReport.filter((r) => r.overflow).length, '화면 오류', shotsReport.reduce((n, r) => n + r.errors, 0));
