...
// 실행: node --experimental-strip-types --test spike/agent-v1-20260925/candidate-v3.5.test.mjs
import test from 'node:test';
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
const A = await import('./candidates/agent-v3.5.ts');
...
test('서비스 설명에 질문 개수 약속 0', () => {
  ...
  assert.equal(A.AGENT_VERSION, 'echo-agent-v3.5');
});
...
// ── v3.5(대표 「v3.5 FINAL REVALIDATION → ROLE LOCK」 · run 35 실제 결함 재현). 모델 자리 = 대본(서버 규칙만).
test('v3.5 EA-21: 모델이 답이라 해도 짧은 반응·물음·오타 항의는 사실 저장 0(run 35 OpenAI FLOW1-5 · FLOW2-2 · FLOW5-4)', async () => {
  for (const [t, q] of [['딥하네', '딥하네'], ['활동?갑자기?', '활동'], ['적었자네', '적었자네']]) {
    const st = await begun(); const before = active(st);
    const r = await A.runTurn(st, t, scripted({ understand: [{ input_type: 'NORMAL_ANSWER', extracted: [{ purpose: 'relationship_style', note: q, quote: q }] }] }).llm);
    assert.equal(r.response.saved, false, t);
    assert.deepEqual(active(st), before, `${t}: 기존 값 그대로`);
    assert.ok(!active(st).includes(q), `${t}: 사실 0`);
    assert.ok(st.turns.at(-1).user === t, `${t}: 원문은 대화 기록에 남음`);
  }
  const st = await begun();
  const r = await A.runTurn(st, '주말에 한 번 보는 게 좋아요', scripted({ understand: [{ input_type: 'NORMAL_ANSWER', extracted: [{ purpose: 'relationship_style', note: '주말 만남', quote: '주말에 한 번 보는 게 좋아요' }] }] }).llm);
  assert.equal(r.response.saved, true, '보통 답은 그대로 저장');
});

test('v3.5 EA-21 판정: questionOnly · reactionOnly 는 자기 이야기(나·저)가 있으면 해당 없음', () => {
  assert.equal(A.questionOnly('활동?갑자기?'), true); assert.equal(A.questionOnly('나는 주말이 좋은데?'), false); assert.equal(A.questionOnly('주말에 한 번 보는 게 좋아요'), false);
  assert.equal(A.reactionOnly('딥하네'), true); assert.equal(A.reactionOnly('저는 좋네'), false); assert.equal(A.reactionOnly('조용한 사람이 좋네'), false);
});

test('v3.5 EA-23: 도움 요청에서 모델이 계속 빈 답이어도 일반 듣기 문장 0 — 예시로 답함(run 35 Claude F4-3)', async () => {
  const st = await begun();
  const empty = () => ({ reply: '', question: '', purpose: '' });
  const r = await A.runTurn(st, '예를 들면?', scripted({ understand: [{ input_type: 'HELP' }], speakOverride: empty }).llm);
  const said = [r.response.reply, r.response.question].join(' ');
  assert.ok(!/이어서 편하게/.test(said), said);
  assert.match(r.response.reply, /예를 들면/);
  assert.equal(r.response.saved, false);
});

test('v3.5 MS-04: 「그런 뜻 아니야」 턴에 방금 들은 뜻을 다시 확인하는 받아주기는 통과 못 함(run 35 OpenAI H_REJECT)', async () => {
  const st = await begun({ first: '다정한 사람이 좋아요', firstU: { input_type: 'NORMAL_ANSWER', extracted: [{ purpose: 'attraction_comfort', note: '다정한 사람', quote: '다정한 사람' }] } });
  const bad = (input) => (input.action === 'REPAIR' ? { reply: '말씀하신 다정한 사람이 좋다는 점, 잘 알겠습니다.', question: '', purpose: '' } : null);
  const r = await A.runTurn(st, '그런 뜻 아니야', scripted({ understand: [{ input_type: 'REJECTION' }], speakOverride: bad }).llm);
  assert.ok(!/다정한\s*사람이\s*좋다는/.test(r.response.reply), r.response.reply);
  assert.ok(r.obs.retry.some((x) => /restates_rejected/.test(x)));
});

test('v3.5 MS-05: 사용자 말을 「내가 적은」으로 쓰는 받아주기는 다시 청함(run 35 OpenAI CEO_COMPLAINT)', async () => {
  const st = await begun();
  let k = 0; const o = (input) => (input.action === 'REPAIR' && k++ === 0 ? { reply: "내가 적은 '편하게 만날 사람'이라는 말을 놓쳤네.", question: '', purpose: '' } : null);
  const r = await A.runTurn(st, '내가 적는거랑 상관없이 질문하네', scripted({ understand: [{ input_type: 'COMPLAINT' }], speakOverride: o }).llm);
  assert.ok(!/내가 적은/.test(r.response.reply), r.response.reply);
  assert.ok(r.obs.retry.some((x) => /speaker/.test(x)));
});

test('v3.5 사람다움 계약: 「~군요」가 앞 받아주기 둘에 이어 또 나오면 한 번 다시 청하고, 그래도 같으면 그대로 둔다(완화 · 서버 문장 대체 0)', async () => {
  const st = await begun();
  const always = () => ({ reply: '그런 사람이 좋으시군요.', question: '다른 이야기는 어때요?', purpose: '' });
  for (const t of ['조용한 사람', '말이 잘 통하는 사람', '약속 잘 지키는 사람']) await A.runTurn(st, t, scripted({ understand: [{ input_type: 'NORMAL_ANSWER' }], speakOverride: always }).llm);
  const r = await A.runTurn(st, '거짓말 안 하는 사람', scripted({ understand: [{ input_type: 'NORMAL_ANSWER' }], speakOverride: always }).llm);
  assert.ok(r.obs.retry.some((x) => /repeat_ending/.test(x)), JSON.stringify(r.obs.retry));
  assert.equal(r.response.reply, '그런 사람이 좋으시군요.', '두 번째도 같으면 그대로(완화)');
});

test('v3.5 사람다움 계약: 사용자가 알려 준 것이 없는데 「얘기해 줘서 고마워」 → 다시 청함(run 35 OpenAI CEO_META)', async () => {
  const st = await begun();
  let k = 0; const o = (input) => (input.action === 'ANSWER_USER' && k++ === 0 ? { reply: '질문이 궁금했구나. 얘기해 줘서 고마워.', question: '', purpose: '' } : null);
  const r = await A.runTurn(st, '질문이 뭐야?', scripted({ understand: [{ input_type: 'META_QUESTION' }], speakOverride: o }).llm);
  assert.ok(!/고마워/.test(r.response.reply), r.response.reply);
});

test('v3.5 EA-22: 다시 만들기가 실패해도 최신 정정은 서버가 뜻 그대로 소개에 한 줄로 넣는다 · politeLine 끝맺음만 바꿈', () => {
  assert.equal(A.politeLine('주말에 한 번 보는 게 좋아', 'polite'), '주말에 한 번 보는 게 좋아요.');
  assert.equal(A.politeLine('주말에 보는 게 좋다는 뜻이야', 'polite'), '주말에 보는 게 좋다고 생각해요.');
  assert.equal(A.politeLine('주말에 보는 게 좋다', 'formal'), '주말에 보는 게 좋다고 생각합니다.');
  assert.equal(A.politeLine('응', 'polite'), null);
});
