...
export function applyTurn(st: AgentState, latest: string, llmOut: Parsed, opts: { limitReached?: boolean } = {}): TurnResponse {
  ...
  if (out.kind === "correction" || out.kind === "repair") {
    ...
    if (out.kind === "correction") {
      ...
      if (sup.length && !kept.some((k) => k.turn === turn.n) && squash(rest).length >= 4) {
        const pid = sup[0];
...
async function refreshIntro(st: AgentState, llm: Llm, obs: Obs) {
  ...
  if (r.limited && st.intro) { st.intro = { ...st.intro, status: "failed", lines: [], error: "limit" }; }
