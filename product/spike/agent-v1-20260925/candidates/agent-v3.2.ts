...
export const AGENT_VERSION = "echo-agent-v3.2"; // v3.0(2026-09-26 대표 「FINAL PRODUCT/AGENT IMPLEMENTATION DIRECTIVE」): TURN CONTRACT — 이해(모델 후보) → 서버 상태 반영 → 서버 행동 결정(Action Router) → 말하기(모델 후보) → 서버 검사. 질문 개수 목표 0(목적 5개는 서버 안의 알아 갈 것 목록일 뿐) · 메타·불만·주제 이동은 행동으로 처리(문장 패턴 추가 0) · 끝내기는 믿을 만한 정보가 모였을 때 · 실패는 상태 보존. 상태 엔진(정정·거절·정보 상태·계보·소개 검사)은 v2.12 그대로.
...
export function roleOfBasis(st: AgentState, basis: string): SemanticRole {
  ...
  return semanticRole(hit ? hit.quote : basis);
}
...
export function cleanIntro(st: AgentState, raw: unknown): { lines: IntroLine[]; dropped: Record<string, number> } {
  ...
  for (const l of Array.isArray(raw) ? raw as Json[] : []) {
    ...
    if (isSelfClaim(text) && !["SELF_TRAIT", "USER_BEHAVIOR"].includes(roleOfBasis(st, basis))) { drop("role_reversal"); continue; } // v3.1 P0-5: 사용자 반말 원문을 끝만 바꿔 그대로 옮긴 문장(원문 누출)
...
const GUARD_INPUT: Record<string, InputType> = { correction_lead: "CORRECTION", skip_request: "SKIP", help_request: "HELP", past_reference: "ALREADY_ANSWERED", fatigue: "COMPLAINT", pause_or_end: "END_INTENT" };
