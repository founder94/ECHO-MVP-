...
export function applyTurn(st: AgentState, latest: string, llmOut: Parsed, opts: { limitReached?: boolean } = {}): TurnResponse {
  ...
  if (st.seed?.asked && st.seed.answered_turn == null && st.current?.text === st.seed.bridge) {
    ...
    if (out.kind === "correction" || /^\s*(아니|아냐|아뇨|전혀|오히려)|안\s*맞|다른데|반대|그렇지\s*않/.test(text)) st.seed.rejected = true;
  }
  st.turns.push(turn);
  ...
  if (question && (decision === "core" || decision === "core_relabeled" || decision === "clarify") && semanticReask(st, question, st.asked.slice(0, -1))) { st.asked.pop(); st.current = null; question = null; decision = "finish"; turn.dropped = "semantic_reask"; }
...
export async function runTurn(st: AgentState, latest: string, llm: Llm): Promise<RunResult> {
  ...
  if (st.seed?.rejected || (st.seed?.asked && st.seed.answered_turn != null)) out = { ...out, reply: out.reply.split(/(?<=[.!~…])\s+/).filter((x) => !(seedIn(st, x) && (st.seed!.rejected || !userSaid(st, st.seed!.phrase)))).join(" ").trim() }; // v2.10: 결과 해석을 사실처럼 되풀이하는 받아주기 문장은 뺀다
