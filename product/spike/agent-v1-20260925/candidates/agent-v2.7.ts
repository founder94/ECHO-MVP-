...
const CORRECTION_LEAD_PART = /^\s*(아니(요|야)?|아냐|그게\s*아니(라|고|야)?|그런\s*(뜻|말)\s*(이\s*)?아니(야|에요|고|라)?|조금\s*달라(요)?|좀\s*달라(요)?)(?=[\s,.!~]|$)[\s,.!~]*/;
...
export function guardKind(text: string, kind: Kind): { kind: Kind; rule: string | null } {
  ...
  if (kind === "repair") return !PAST_REF.test(text) && !FATIGUE.test(text) && isCorrectionLead(text) ? { kind: "correction", rule: "correction_lead" } : { kind, rule: null };
...
const EVALUATIVE_SENTENCE = /좋은\s*(방법|선택|생각)|멋지|훌륭|자연스러워요|좋네요|좋아요[.!~]?\s*$|그랬군요|힘드셨|대단|인상적|힘들었(구나|군요|겠)|불편하셨|불편했(구나|군요|겠)|부담스러우셨|부담스러웠(구나|군요|겠)|속상하셨|속상했(구나|군요)|서운하셨|아쉬운\s*마음/;
/** 「선호」만 서버가 쉬운 말로 바꾼다(뜻이 같다). 다른 딱딱한 말은 AI 에 한 번 다시 청한다. */
export function softenQuestion(q: string): string {
  return q.replace(/선호(?=[하합해했한할함])/g, "좋아").replace(/선호/g, "좋아하는 것"); // v2.7: 「선호합니다·선호해요·선호하는지」 모두
}
...
export function applyTurn(st: AgentState, latest: string, llmOut: Parsed, opts: { limitReached?: boolean } = {}): TurnResponse {
  ...
  if (out.kind === "correction" || out.kind === "repair") {
    ...
    if (out.kind === "correction") {
      ...
      const topic = correctionTopic(text); const prev = st.turns.length >= 2 ? st.turns[st.turns.length - 2].n : null;
      if (topic && prev != null) for (const id of PIDS) for (const i of st.slots[id].items) if (i.status === "CONFIRMED" && i.turn === prev && squash(i.quote).includes(topic)) { i.status = "SUPERSEDED"; i.superseded_at = now(); n++; }
...
export function cleanIntro(st: AgentState, raw: unknown): { lines: IntroLine[]; dropped: Record<string, number> } {
  ...
  for (const l of Array.isArray(raw) ? raw as Json[] : []) {
    ...
    if (superseded.some((q) => squash(text).includes(q) || b.includes(q))) { drop("superseded"); continue; } // v2.6: 정정으로 밀린 옛 값 문장은 소개에 쓰지 않는다
    ...
    lines.push({ text: softenQuestion(text), basis }); total += added; // v2.7: 소개에도 「선호」 대신 쉬운 말
...
export async function runTurn(st: AgentState, latest: string, llm: Llm): Promise<RunResult> {
  ...
  out = { ...out, reply: softenQuestion(dropEvaluative(out.reply)), next: { ...out.next, question: out.next.question ? softenQuestion(out.next.question) : out.next.question } }; // v2.5 // 반응 칸의 물음표는 질문 수를 늘리므로 화면에 물음표로 내지 않는다
  if (after) {
    st.after_turns++;
    st.phase = "post";
