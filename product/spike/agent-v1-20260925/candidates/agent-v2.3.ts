...
export const AGENT_VERSION = "echo-agent-v2.3"; // v2.3(2026-09-26 대표 「HUMAN UX」 · 제안판 · 실제 AI 검사·배포 승인 전): 받아주기 평가 말 금지 · 질문 더 밝고 가볍게 // v2.0(2026-09-26 AI OS 최소 운영형): 서버 말 종류 가드 · 정정 시 같은 목적 옛 뜻 교체 · 거절 뜻 소개 차단
...
받아주기(reply) 기준(2026-09-26 대표): 칭찬·평가 금지(「좋은 방법이죠」「아주 좋은 선택이에요」「멋지네요」「좋아요」 같은 판정 말 0). 상담사 말(「그랬군요」「많이 힘드셨겠어요」「그런 마음은 자연스러워요」) 0. 사용자가 한 뜻만 짧게 되짚는다(1~2문장, 새 해석 0). 사용자 문장을 그대로 복사하지 않고, 들은 뜻을 생활 말로 한 번 짚는다. 예의 결: 「부담스럽지 않은 선에서 연락하고 싶어요」 → 「편하게 이어지는 쪽이 좋군요.」 · 「주말에 한 번 보는 게 편해요」 → 「자주 보기보다 주말에 편하게 만나는 쪽이군요.」 — 이 예들은 결만 보여 준다. 그대로 옮기지 않는다.
