...
export const AGENT_VERSION = "echo-agent-v3.5"; // v3.0(2026-09-26 대표 「FINAL PRODUCT/AGENT IMPLEMENTATION DIRECTIVE」): TURN CONTRACT — 이해(모델 후보) → 서버 상태 반영 → 서버 행동 결정(Action Router) → 말하기(모델 후보) → 서버 검사. 질문 개수 목표 0(목적 5개는 서버 안의 알아 갈 것 목록일 뿐) · 메타·불만·주제 이동은 행동으로 처리(문장 패턴 추가 0) · 끝내기는 믿을 만한 정보가 모였을 때 · 실패는 상태 보존. 상태 엔진(정정·거절·정보 상태·계보·소개 검사)은 v2.12 그대로.
...
const PAST_REF = /(아까|이미|전에|앞에서|방금)\s*.{0,10}(말했|말한|적었|적은|얘기했|얘기한|했잖|했는데)|말했잖|적었잖|했잖아|(말|적|얘기)(했|한|은)\s*(거|것)\s*같은데|왜\s*(또|자꾸|계속)\s*(물어|묻)|또\s*물어|같은\s*(걸|거|질문)\s*(또|다시)|(적었|말했|얘기했|했)\s*(자네|잔아|쟎아|자나)(?![가-힣])/; // v3.5 EA-21: 「적었자네」(적었잖아 오타) — run 35 FLOW1-5
...
export function guardKind(text: string, kind: Kind): { kind: Kind; rule: string | null } {
  ...
  // v3.5 EA-21(run 35): 모델이 답(answer)이라 해도 서버가 저장 직전 「자기 정보인가」를 다시 본다 — 물음으로만 된 짧은 말(「활동?갑자기?」)은 사용자 질문,
  // 짧은 감탄·평가 반응(「딥하네」)은 질문에 대한 반응이다. 둘 다 사실 저장 0 · 원문은 대화 기록(turns)에 남는다.
  if (questionOnly(text)) return { kind: "ask", rule: "question_not_answer" };
  if (reactionOnly(text)) return { kind: "unsure", rule: "reaction_not_fact" };
  return { kind, rule: null };
}
const SELF_WORD = /(^|\s)(나|난|내가|나는|저|전|제가|저는|내|제)(?=\s|$)/;
export function questionOnly(text: string): boolean {
  const t = String(text ?? "").trim(); const parts = t.split(/(?<=[?？])/).map((x) => x.trim()).filter(Boolean);
  return parts.length > 0 && parts.every((x) => /[?？]$/.test(x)) && squash(t).length <= 20 && !SELF_WORD.test(t);
}
export function reactionOnly(text: string): boolean {
  const s = squash(text).replace(/[!~.?？…]+$/, "");
  return s.length >= 1 && s.length <= 4 && /(네|네요|구나|군|군요|다니|잖아|자네)$/.test(s) && !SELF_WORD.test(text);
}
...
- ECHO 사람다움 계약(v3.5 · 업체 공통): 받아주기는 방금 말의 한 부분을 자연스럽게 잡는다 · recent_replies 와 같은 끝맺음(~군요·~구나)을 연달아 쓰지 않는다 · 사용자 문장을 그대로 되풀이하지 않는다 · 앞 턴이 질문뿐이었으면 이번에는 짧게 받아준 뒤 묻는다 · gaps 의 label 을 문장에 그대로 옮기지 않는다 · 사용자가 알려 주지 않은 것에 고마워하지 않는다 · 사용자의 말은 「말씀하신·적어 주신」으로 짚고 「내가 적은」처럼 쓰지 않는다 · 사용자가 「그런 뜻 아니야」라고 하면 방금 그 뜻을 다시 확인하지 않는다.
...
export const SPEAK_FEEDBACK: Record<string, string> = {
  ...
  speaker: "reply 가 사용자의 말을 ECHO 자신의 말처럼 표현했다(「내가 적은」). 사용자 말은 「말씀하신·적어 주신」으로 짚는다.",
  restates_rejected: "사용자가 방금 「그런 뜻 아니야」라고 했는데 reply 가 그 뜻을 다시 확인했다. 그 뜻을 말하지 말고, 무엇을 말한 건지 짧게 여쭌다.",
  repeat_ending: "받아주기가 앞 받아주기들과 같은 끝맺음(~군요·~구나)을 되풀이한다. 다른 자연스러운 말로 짧게 받는다.",
  unsupported_thanks: "사용자가 알려 준 것이 없는데 고마워했다. 그 말을 빼고 사용자의 말에 바로 답한다.",
  no_ack: "앞 턴에 이어 또 질문만 있다. 방금 말의 한 부분을 짧게 받아준 뒤 묻는다.",
};
const SPEAKER_SELF_CLAIM = /(내가|제가)\s*(적은|쓴|써\s*준|말한|얘기한|했던\s*말)/;
const ENDING_FAMILY = /(군요|구나|군)[.!~…]*$/;
const THANKS_FOR_TELLING = /(얘기해|말해|알려|말씀해)\s*(줘서|주셔서)\s*(고마|감사)/;
export const SOFT_PROBLEMS = new Set(["repeat_ending", "unsupported_thanks", "no_ack"]);
export function speakProblem(st: AgentState, action: Action, s: Spoken, gaps: string[], ctx: { text: string; prevQuestion: string | null; rejectedNow?: string[] }): string {
  ...
  // v3.5 MS-05: ECHO 가 사용자 말을 「내가 적은·제가 말한」처럼 자기 말로 표현(화자 혼동) — run 35 CEO_COMPLAINT
  if (s.reply && SPEAKER_SELF_CLAIM.test(s.reply)) return "speaker";
  // v3.5 MS-04: 사용자가 「그런 뜻 아니야」라고 한 턴에 받아주기가 방금 거절된 뜻(앞 답에서 들은 말)을 다시 확인 — run 35 H_REJECT
  if (s.reply && ctx.rejectedNow?.some((r) => r.length >= 3 && squash(s.reply).includes(r))) return "restates_rejected";
  // v3.5 사람다움 계약(업체 공통 · 검출 → 한 번 다시 청함 · 그래도 같으면 그대로 둔다 = 완화, 서버 문장으로 바꾸지 않음)
  if (s.reply && ENDING_FAMILY.test(s.reply.trim()) && st.turns.slice(-4, -1).filter((t) => t.reply && ENDING_FAMILY.test(t.reply.trim())).length >= 2) return "repeat_ending";
  if (s.reply && THANKS_FOR_TELLING.test(s.reply) && (/[?？]/.test(ctx.text) || squash(ctx.text).length < 4)) return "unsupported_thanks";
  if ((action === "ASK_GAP" || action === "FOLLOW") && !s.reply && s.question && squash(ctx.text).length >= 6) { const prev = st.turns[st.turns.length - 2]; if (prev && !prev.reply && prev.question) return "no_ack"; }
...
export type RecoveryType = "COMPLAINT" | "META" | "HELP" | "QUESTION_GENERATION_FAILURE" | "EMPTY_REPLY" | "PARSE_FAILURE" | "FOLLOW_UP_FAILURE" | "VALIDATION_FAILURE";
...
export function recoveryType(action: Action, why: string): RecoveryType {
  ...
  if (action === "EXPLAIN") return "HELP"; // v3.5 EA-23: 도움 요청(「예를 들면?」)은 일반 듣기 문장으로 떨어지지 않는다
...
export const RECOVERY_BRIEF: Record<RecoveryType, string> = {
  ...
  HELP: "사용자가 질문 뜻이나 예를 물었다. reply 에 지금 질문(current_question)의 뜻을 짧게 풀고 예 1~2개(개념만 · 답을 대신 써 주지 않음). question 은 비워도 된다.",
...
// v3.5 EA-23: 도움 요청 최후 서버 문장 — 지금 묻던 목적의 예(개념만 · 사용자 사실 아님 · 저장 0).
const HELP_EXAMPLES: Record<string, string> = {
  relationship_intent: "친구처럼 편하게 지낼 사람, 천천히 연애로 이어질 사람", attraction_comfort: "말이 잘 통하는 사람, 조용히 있어도 편한 사람",
  values_character: "약속을 잘 지키는지, 솔직한지", relationship_style: "연락은 하루 한두 번, 주말에 한 번 만나기", boundaries: "거짓말은 싫다, 연락이 너무 뜸한 건 불편하다",
};
export function helpLine(st: AgentState): string {
  const ex = HELP_EXAMPLES[st.current?.purpose ?? ""] ?? HELP_EXAMPLES[openPurposes(st)[0] ?? "relationship_intent"];
  return st.tone === "formal" ? `예를 들면 「${ex}」 같은 것입니다. 떠오르는 대로 말씀해 주시면 됩니다.` : st.tone === "casual" ? `예를 들면 「${ex}」 같은 거야. 떠오르는 대로 말해 줘.` : `예를 들면 「${ex}」 같은 거예요. 떠오르는 대로 말해 주셔도 돼요.`;
}
...
const GUARD_INPUT: Record<string, InputType> = { question_not_answer: "USER_QUESTION", reaction_not_fact: "UNSURE", complaint_not_correction: "COMPLAINT", complaint_to_echo: "COMPLAINT", uncorroborated_correction: "COMPLAINT", correction_lead: "CORRECTION", skip_request: "SKIP", help_request: "HELP", past_reference: "ALREADY_ANSWERED", fatigue: "COMPLAINT", pause_or_end: "END_INTENT" };
...
const REJECT_LEAD = /^\s*(아니(요|야)?|아냐|그게\s*아니|그런\s*(뜻|말)\s*(이\s*)?아니)/;
async function speak(st: AgentState, action: Action, text: string, gaps: string[], prevQuestion: string | null, about: string, llm: Llm, obs: Obs, opt: { after?: boolean; noQuestion?: boolean } = {}): Promise<Spoken & { recovery?: string }> {
  // v3.5 MS-04: 이번 말이 거절(「그런 뜻 아니야」·「아니」로 시작하는 불만)이면, 바로 앞 사용자 답에서 들은 말을 「방금 거절된 뜻」으로 본다(받아주기에 다시 쓰면 안 됨).
  const prevTurn = st.turns[st.turns.length - 2];
  const rejectedNow = action === "REPAIR" && REJECT_LEAD.test(text) && prevTurn ? PIDS.flatMap((id) => st.slots[id].items.filter((i) => i.turn === prevTurn.n).flatMap((i) => [squash(i.quote), squash(i.note)])).filter((x) => x.length >= 3) : [];
  ...
  const check = (c: { spoken: Spoken; salvaged: boolean }): { spoken: Spoken; why: string } => {
    let why = speakProblem(st, action, c.spoken, gaps, { text, prevQuestion, rejectedNow });
    if (noQ && QUESTION_PROBLEMS.has(why)) why = c.spoken.reply ? "" : "empty_reply"; // 질문 0 인 턴은 질문 약속을 따지지 않는다(받아주기는 필수)
    if (why && c.salvaged && QUESTION_PROBLEMS.has(why)) { const bare = { ...c.spoken, question: "", purpose: "" }; const w2 = speakProblem(st, action, bare, gaps, { text, prevQuestion, rejectedNow }); note(obs, `speak_salvage_dropped:${why}`); return { spoken: bare, why: w2 }; }
  ...
  for (let i = 0; i < MAX_CALLS_PER_TURN; i++) {
    ...
    if (i + 1 >= MAX_CALLS_PER_TURN && SOFT_PROBLEMS.has(why)) { note(obs, `humanity_soft_kept:${why}`); obs.retry.push(`speak_${why}:kept`); return r.spoken; } // 완화: 두 번째도 같으면 그대로 쓴다
  ...
  if (why !== "provider") {
    try {
      ...
      if (p) {
        const c = clean(p);
        let sp = c.spoken; let w = speakProblem(st, action, sp, gaps, { text, prevQuestion, rejectedNow }); if (SOFT_PROBLEMS.has(w)) w = "";
        // 복구에서는 질문이 필수가 아니다: 질문이 문제면 질문만 버리고, 남은 받아주기가 검사(말투·빈 답)를 통과하면 쓴다.
        if (w && QUESTION_PROBLEMS.has(w) && sp.reply) { sp = { ...sp, question: "", purpose: "" }; const w2 = speakProblem(st, action, sp, gaps, { text, prevQuestion, rejectedNow }); w = QUESTION_PROBLEMS.has(w2) ? "" : w2; }
  ...
  const reply = type === "COMPLAINT" ? (opt.after ? AFTER_REPAIR_LINE[st.tone] : repairLine(st, st.turns.length)) : type === "META" ? META_LINE[st.tone] : type === "HELP" ? (usable(last) ? last!.reply : helpLine(st)) : usable(last) ? last!.reply : LISTEN_LINE[st.tone];
...
async function runTurnInner(st: AgentState, latest: string, llm: Llm): Promise<RunResult> {
  ...
  if (!reply && !question) { const t = recoveryType(action, "empty_turn"); reply = t === "COMPLAINT" ? repairLine(st, turn.n) : t === "META" ? META_LINE[st.tone] : t === "HELP" ? helpLine(st) : LISTEN_LINE[st.tone]; turn.recovery = `${t}:final_guard`; note(obs, `recovery:${t}:final_guard`); }
...
async function reconcileIntro(st: AgentState, base: IntroLine[], llm: Llm, obs: Obs) {
  ...
  // v3.5 EA-22(run 35 Claude F6): 다시 만들기 뒤에도 최신 정정이 빠졌으면 서버가 그 말로 안전하게 한 줄을 만든다(뜻 추가 0 · 끝맺음만 존댓말로).
  for (const q of latestCorrections(st).filter((q) => !lines.some((l) => covers(l, q)))) {
    const text = politeLine(q, st.tone); if (!text) continue;
    const c = cleanIntro(st, [{ text, basis: q }]); if (c.lines.length === 1) { lines = [...lines, c.lines[0]]; note(obs, "intro_server_line"); }
    const keep = new Set(latestCorrections(st).map(squash)); const len = (ls: IntroLine[]) => ls.map((l) => l.text).join(" ").length;
    while ((len(lines) > INTRO_MAX || lines.length > INTRO_MAX_LINES) && lines.length > 1) { const k = lines.findIndex((l) => !keep.has(squash(l.basis))); if (k < 0) break; lines.splice(k, 1); }
  }
...
/** v3.5 EA-22: 정정 원문을 소개 한 줄로(뜻 그대로 · 끝맺음만). 바꿀 수 없는 모양이면 null(억지로 만들지 않음). */
export function politeLine(q: string, tone: Tone): string | null {
  const t = String(q ?? "").trim().replace(/[.!~…]+$/, "").replace(/는\s*(뜻|말|얘기)(이야|이에요|예요|이다)?$/, "").trim();
  if (squash(t).length < 4) return null;
  if (POLITE_END.test(t)) return `${t}.`;
  if (/다$/.test(t)) return `${t}고 ${tone === "formal" ? "생각합니다" : "생각해요"}.`;
  if (/이야$/.test(t)) return t.replace(/이야$/, tone === "formal" ? "입니다." : "이에요.");
  if (/야$/.test(t)) return t.replace(/야$/, tone === "formal" ? "입니다." : "예요.");
  if (/(어|아|해|돼|지|워)$/.test(t)) return `${t}요.`;
  return null;
}
