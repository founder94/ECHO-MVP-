...
export const AGENT_VERSION = "echo-agent-v3.3"; // v3.0(2026-09-26 대표 「FINAL PRODUCT/AGENT IMPLEMENTATION DIRECTIVE」): TURN CONTRACT — 이해(모델 후보) → 서버 상태 반영 → 서버 행동 결정(Action Router) → 말하기(모델 후보) → 서버 검사. 질문 개수 목표 0(목적 5개는 서버 안의 알아 갈 것 목록일 뿐) · 메타·불만·주제 이동은 행동으로 처리(문장 패턴 추가 0) · 끝내기는 믿을 만한 정보가 모였을 때 · 실패는 상태 보존. 상태 엔진(정정·거절·정보 상태·계보·소개 검사)은 v2.12 그대로.
...
export function applyState(st: AgentState, latest: string, llmOut: ParsedU): { turn: TurnRec; extracted: { purpose: string; note: string }[]; recovered: string[] } {
  ...
  const notAFact = (q: string) => !!redirectOf(q) || (rest !== null && !rest.includes(squash(q))) || (redirectTurn && wholeUtterance(q)) || !isUserOwn(text, q); // v3.3 ②: 남의 말·인용은 사실 0
...
const GUARD_INPUT: Record<string, InputType> = { complaint_not_correction: "COMPLAINT", correction_lead: "CORRECTION", skip_request: "SKIP", help_request: "HELP", past_reference: "ALREADY_ANSWERED", fatigue: "COMPLAINT", pause_or_end: "END_INTENT" };
