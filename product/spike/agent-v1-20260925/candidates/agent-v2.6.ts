...
export function softenQuestion(q: string): string {
  return q.replace(/선호하(시나요|세요|나요|는지|는|시는|십니까|해요)/g, (_, e: string) => `좋아하${e}`).replace(/선호/g, "좋아하는 것");
}
...
export async function runTurn(st: AgentState, latest: string, llm: Llm): Promise<RunResult> {
  ...
  out = { ...out, reply: dropEvaluative(out.reply), next: { ...out.next, question: out.next.question ? softenQuestion(out.next.question) : out.next.question } }; // v2.5 // 반응 칸의 물음표는 질문 수를 늘리므로 화면에 물음표로 내지 않는다
  if (after) {
    st.after_turns++;
    st.phase = "post";
