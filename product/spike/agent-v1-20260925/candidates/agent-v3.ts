...
export const AGENT_VERSION = "echo-agent-v3.0"; // v3.0(2026-09-26 대표 「FINAL PRODUCT/AGENT IMPLEMENTATION DIRECTIVE」): TURN CONTRACT — 이해(모델 후보) → 서버 상태 반영 → 서버 행동 결정(Action Router) → 말하기(모델 후보) → 서버 검사. 질문 개수 목표 0(목적 5개는 서버 안의 알아 갈 것 목록일 뿐) · 메타·불만·주제 이동은 행동으로 처리(문장 패턴 추가 0) · 끝내기는 믿을 만한 정보가 모였을 때 · 실패는 상태 보존. 상태 엔진(정정·거절·정보 상태·계보·소개 검사)은 v2.12 그대로.
...
export function guardKind(text: string, kind: Kind): { kind: Kind; rule: string | null } {
  ...
  if (kind === "ask" && isCorrectionLead(text)) return { kind: "correction", rule: "correction_lead" }; // v3: 같은 정정 머리 규칙을 ask 에도(run 32 F5 「그런 뜻 아니야, 연락 얘기였어 …」가 ask 로 읽힘) — 새 패턴 0
...
- END_INTENT: 그만하고 싶다·지쳤다·질문이 너무 많다.
...
extracted: 사용자가 직접 한 말에서만, 목적 id(relationship_intent·attraction_comfort·values_character·relationship_style·boundaries) 별로 {purpose, note, quote}. note = 짧은 요약(사용자 뜻 그대로, 해석·평가 0), quote = 사용자가 친 글자를 오타·띄어쓰기까지 그대로 복사한 일부. 질문·불만·메타 문장 자체는 넣지 않는다. 만남과 상관없는 이야기(회사 일 등)는 넣지 않는다. 앞선 사용자 말(recent)에 있는데 heard 에 아직 없는 정보는 그 말에서 quote 를 복사해 넣어도 된다.
...
- FOLLOW: 사용자가 자기 이야기(일·요즘 일·잡담)를 하고 있다. 그 이야기를 따라간다 — 관계 질문으로 억지로 끌고 오지 않는다. question 은 그 이야기에서 이어지는 질문 하나. bridge_ok 가 true 면 그 이야기와 사람·만남이 자연스럽게 닿는 쪽으로 물어도 된다.
...
export function applyState(st: AgentState, latest: string, llmOut: ParsedU): { turn: TurnRec; extracted: { purpose: string; note: string }[]; recovered: string[] } {
  ...
  const notAFact = (q: string) => !!redirectOf(q) || (rest !== null && !rest.includes(squash(q)));
  ...
  // v3: 원문을 답으로 남기는 것은 답·새 사실(모델이 사실을 못 뽑았을 때)만, 지금 목적 질문(core)이나 듣고 있던 목적에만 — 잡담·불만·메타는 남기지 않는다.
  const rawPid = st.current ? st.current.purpose : st.listening ?? null; // v2.13 보존: 열린 질문·질문 없이 듣기 뒤의 답도 잃지 않는다
  if (out.kind === "answer" && llmOut.raw_ok && rawPid && st.slots[rawPid]?.status === "UNKNOWN" && !kept.some((k) => k.purpose === rawPid && k.turn === turn.n)
...
// 충분 = 원하는 만남 확인 + 다섯 목적 중 넷이 확인·넘김 · 또는 물을 목적이 없음 · 또는 목적 질문 상한(MAX_CORE_QUESTIONS, 약속 아님 · 상한일 뿐).
export function coverage(st: AgentState) { const known = PIDS.filter((id) => st.slots[id].status !== "UNKNOWN").length; return { known, of: PIDS.length, intent: st.slots.relationship_intent.status === "CONFIRMED" }; }
export function enoughKnown(st: AgentState): boolean { const c = coverage(st); return (c.intent && c.known >= 4) || !openPurposes(st).length || coreAsked(st).length >= MAX_CORE_QUESTIONS; }
...
export function route(st: AgentState, it: InputType, opts: { limitReached?: boolean } = {}): Action {
  ...
  if (it === "HELP") return st.current && st.slots[st.current.purpose]?.status === "UNKNOWN" && (st.current.helps ?? 0) < MAX_HELP_PER_QUESTION ? "EXPLAIN" : enoughKnown(st) ? "CLOSE" : "ASK_GAP";
  // 사주·타로에서 온 대화: 첫 답 뒤 한 번, 서버가 정한 다리 문장(결과 = 참고 · 사용자 사실 아님).
  if (st.seed && !st.seed.asked && st.turns.length >= 1) return "BRIDGE";
  return enoughKnown(st) ? "CLOSE" : "ASK_GAP";
}
...
export function speakProblem(st: AgentState, action: Action, s: Spoken, gaps: string[], ctx: { text: string; prevQuestion: string | null }): string {
  ...
  const q = s.question;
  if (!q) return "";
  if ((q.match(/[?？]/g) ?? []).length > 1) return "multi_question";
...
// 말하기가 두 번 다 실패했을 때: 질문을 빼고(끝내지 않음 · 상태 그대로) 받아주기만, 그것도 없으면 이어서 말하라는 짧은 안내(질문 아님 · 정책 문장).
...
const GUARD_INPUT: Record<string, InputType> = { correction_lead: "CORRECTION", skip_request: "SKIP", help_request: "HELP", past_reference: "ALREADY_ANSWERED", fatigue: "COMPLAINT" };
...
async function speak(st: AgentState, action: Action, text: string, gaps: string[], prevQuestion: string | null, about: string, llm: Llm, obs: Obs): Promise<Spoken> {
  const input: Json = {
    action, latest: text, about,
  ...
  const userCtx = [text, ...st.turns.slice(-3, -1).map((t) => t.user)].join(" ");
  const clean = (s: Spoken): Spoken => {
    let reply = dropQuestions(s.reply);
    reply = softenQuestion(dropAssumedEmotion(dropEvaluative(reply), userCtx));
    if (action !== "ANSWER_USER" && action !== "EXPLAIN") reply = dropUngrounded(reply, userCtx);
    if (st.seed?.rejected || st.seed?.asked) reply = reply.split(/(?<=[.!~…])\s+/).filter((x) => !(seedIn(st, x) && (st.seed!.rejected || !userSaid(st, st.seed!.phrase)))).join(" ").trim();
    if (BANNED_WORDS.test(reply) || leaksId(reply)) reply = "";
    const question = s.question && !BANNED_WORDS.test(s.question) && !leaksId(s.question) ? softenQuestion(s.question) : "";
    return { reply, question: action === "AFTER_ACK" || action === "BRIDGE" ? "" : question, purpose: s.purpose };
  };
  ...
  for (let i = 0; i < MAX_CALLS_PER_TURN; i++) {
    ...
    catch { obs.retry.push("speak_provider"); break; }
    const p = parseSpeak(raw);
    if (!p) { why = "format"; obs.retry.push("speak_format"); continue; }
    const c = clean(p); last = c;
    why = speakProblem(st, action, c, gaps, { text, prevQuestion });
    if (!why) return c;
    obs.retry.push(i + 1 < MAX_CALLS_PER_TURN ? `speak_${why}` : `speak_${why}:dropped`);
  }
  // 두 번째도 문제면: 문제 있는 질문은 빼고(끝내지 않음) 받아주기만 · 받아주기도 없으면 이어서 말하라는 짧은 안내.
  const reply = last?.reply && !(why === "tone" || why === "rejected_meaning") ? last.reply : "";
  return { reply: reply || LISTEN_LINE[st.tone], question: "", purpose: "" };
}
...
export async function runTurn(st: AgentState, latest: string, llm: Llm): Promise<RunResult> {
  ...
  if (after) {
    st.phase = "done";
    const s = await speak(st, "AFTER_ACK", text, [], null, u.about, llm, obs);
    turn.reply = s.reply; turn.question = null; turn.action = "AFTER_ACK"; turn.decision = "after";
    if ((turn.kind === "correction" && (turn.superseded || turn.saved)) || turn.saved) { await refreshIntro(st, llm, obs); await ensureLatestInIntro(st, llm, obs); }
    const profile = matchingProfile(st);
    return { obs, response: { kind: turn.kind, input_type: it, reply: s.reply, question: null, saved: !!turn.saved, extracted: r.extracted, recovered: r.recovered, finish: false, after: true, profile, handoff: matchingHandoff(profile), ...(recovery ? { correction_recovery: true } : {}) } };
  }
  ...
  if (action === "CLOSE") {
    ...
    await ensureLatestInIntro(st, llm, obs);
  ...
  const s = await speak(st, action, text, action === "ASK_GAP" || action === "ACK_CORRECTION" ? gaps : gaps, prevQuestion, u.about, llm, obs);
  ...
  turn.reply = s.reply; turn.question = question; turn.hint = null;
  turn.decision = question ? action.toLowerCase() : `${action.toLowerCase()}_listen`;
  return { obs, response: { kind: turn.kind, input_type: it, action, reply: s.reply, question, saved: !!turn.saved, extracted: r.extracted, recovered: r.recovered, finish: false, question_type: qType, question_purpose: question ? st.current?.purpose ?? null : null } };
}
