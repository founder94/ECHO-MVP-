...
const RECENT_TURNS = 4; // v2.11 비용: 10 → 4(실측: 고정 지시문이 입력의 약 85~90% · 원문은 최근 4턴 + 아래 「아직 저장 안 된 앞선 말」만)
const EARLIER_MAX = 3; const EARLIER_CHARS = 80;
// v2.11: 물음이 있을 때만 서비스 안내(service_facts)를 붙인다(모양만 · 「ask」 답에 필요).
const ASKS_SERVICE = /[?？]|에코|ECHO|서비스|앱|매칭|연결|AI|너는|넌\s|뭐야|뭐예요|누구|어떻게\s*(돼|되|해|하)|왜\s*(물|자꾸|또)/i;
...
- 앞선 말(recent 의 user · earlier_user)에서: heard 에 아직 없는 목적의 정보가 앞선 사용자 말에 있으면 그 말에서 quote 를 복사해 넣는다(놓친 것 되살리기 · 서버가 이 대화의 사용자 말에서 글자를 확인한다).
...
- 질문 말투 기준(묻기 전에 스스로 확인한다): context = 방금 말·앞선 말과 이어진다 · concrete = 가치·방식·스타일·느낌 같은 추상 낱말만으로 묻지 않고 연락·약속·처음 만났을 때·주말처럼 실제 장면을 떠올릴 수 있다 · answerable = 35~52세 보통 사람이 설명 없이 바로 한 줄로 답할 수 있다. 하나라도 아니면 더 쉬운 문장으로 바꿔서 낸다. 짧은 한 문장, 상담·심리검사·면접 말투 금지.
...
JSON 하나로만 답한다: {"kind":"","reply":"","extracted":[{"purpose":"","note":"","quote":""}],"inferred":[{"trait":"","basis":""}],"declared":null,"wrong":[],"next":{"type":"core","purpose":"","question":"","hint":""}}
${toneBlock(tone)}`;
}
...
const INTRO_RULE = `intro: 다른 사람에게 보여 줄 내 소개 초안. 1인칭(「저는」)으로 2~${INTRO_MAX_LINES}문장, 모두 합쳐 ${INTRO_MAX}자 이내. heard 에 있는 사용자 말로만 쓴다(없는 사실·성격 평가·장점 과장·미래 약속 금지, 추측을 사실처럼 쓰지 않는다). 각 문장의 basis 에는 그 문장이 기댄 heard 의 quote 를 글자 그대로 복사한다. 사용자 말을 길게 그대로 옮기지 말고 자연스럽고 담백하게 다듬는다. 연락처·링크·실명·나이 같은 개인 정보는 넣지 않는다. heard 가 비었으면 intro 는 []. rejected 는 사용자가 아니라고 한 뜻이다 — 소개에 쓰지 않는다. latest_corrections 는 사용자가 마지막으로 고친 말이다 — 소개에 반드시 그 고친 뜻을 담고(basis 는 그 quote), 사용자가 고른 말투와 같은 말투로 자연스럽게 다듬는다.
...
export interface IntroDraft { status: "ready" | "failed" | "none"; lines: IntroLine[]; dropped: Record<string, number>; tries: number; error: string | null; failure?: "PROFILE_EMPTY_AFTER_CORRECTION" | "PROFILE_GENERATION_FAILURE" | null; used: "as_is" | "edited" | "own" | null; used_at: string | null }
...
export function turnInput(st: AgentState, latest: string): Json {
  return {
    ...
    // v2.11 Context Memory: 최근 창 밖의 앞선 사용자 말 중 아직 어떤 값으로도 저장되지 않은 답만(「아까 말했는데」 되살리기용 · 저장된 것은 heard 에 있다).
    earlier_user: st.turns.slice(0, -RECENT_TURNS).filter((t) => !t.saved && (t.kind === "answer" || t.kind === "ask" || t.kind === "repair") && squash(t.user).length >= 4).slice(-EARLIER_MAX).map((t) => ({ n: t.n, user: t.user.slice(0, EARLIER_CHARS) })),
    ...
    service_facts: ASKS_SERVICE.test(latest) ? SERVICE_FACTS : [],
...
export function applyTurn(st: AgentState, latest: string, llmOut: Parsed, opts: { limitReached?: boolean } = {}): TurnResponse {
  ...
  if (st.seed?.asked && st.seed.answered_turn == null && st.current?.text === st.seed.bridge) {
    ...
    if (out.kind === "correction" || /^\s*(아니|아냐|아뇨|전혀)|아닌데|아닌\s*것\s*같|오히려|안\s*맞|다른데|반대|그렇지\s*않/.test(text)) st.seed.rejected = true; // v2.11: 「이건 좀 아닌데」「나는 오히려」도
  }
  st.turns.push(turn);
  ...
  if (out.kind !== "stop") {
    for (const m of out.extracted) {
      ...
      // v2.11: 사주·타로 결과에 대한 반박은 앞선 사용자 값을 고친 것이 아니다 — 사용자 직접 말(정정으로 세지 않음 · 결과는 원래 사용자 사실이 아니다).
  ...
  if (out.kind === "correction" || out.kind === "repair") {
    ...
    if (out.kind === "correction") {
      ...
      if (pidRaw && !kept.some((k) => k.turn === turn.n) && squash(rest).length >= 4) {
        const pid = pidRaw;
        st.slots[pid].items.push({ note: rest.slice(0, RAW_NOTE_MAX), quote: rest, turn: turn.n, source: "correction_raw", status: "CONFIRMED", source_type: "USER_CORRECTED", confirmed_at: now() });
...
function finishWith(st: AgentState, raw: unknown) {
  ...
  const closing = o ? softenQuestion(dropAssumedEmotion(dropEvaluative(str(o.closing)), st.turns.map((t) => t.user).join(" "))) : ""; // v2.8: 마무리 말도 사용자 말에 없는 감정은 쓰지 않는다
...
export function retryReason(st: AgentState, out: Parsed, left: string[], after: boolean, text = ""): string {
  // v2.11 비용·품질: 받아주기 속 물음은 다시 청하지 않고 서버가 그 문장만 뺀다(아래 runTurn).
...
export async function runTurn(st: AgentState, latest: string, llm: Llm): Promise<RunResult> {
  ...
  if (/[?？]/.test(out.reply)) out = { ...out, reply: out.reply.split(/(?<=[.!~…?？])\s+/).filter((x) => !/[?？]\s*$/.test(x.trim())).join(" ").replace(/[?？]/g, ".").trim() }; // v2.11: 물음 문장은 빼고(받아주기는 들었다는 신호만), 남은 물음표는 마침표로
  const answeringSeed = !!st.seed?.asked && st.seed.answered_turn == null && st.current?.text === st.seed.bridge; // v2.11: 다리 질문에 답하는 이번 턴도 거른다
  ...
  if (after) {
    ...
    if (last.kind === "correction" && (last.superseded || r.saved)) await refreshIntro(st, llm, obs);
    const profile = matchingProfile(st);
  ...
  if (response.finish) {
    ...
    if ((st.intro?.status === "failed" || !introReflectsLatest(st)) && heardQuoted(st).length) await refreshIntro(st, llm, obs); // v2.8 · v2.10: 최근 정정이 소개에 없으면 기존 상한 안에서 한 번 다시 쓴다: 유효 정보가 있는데 비면 기존 상한 안에서 한 번 다시 쓴다
  }
  return { obs, response };
}
...
async function refreshIntro(st: AgentState, llm: Llm, obs: Obs) {
  ...
  // v2.11: 다시 쓰기가 비어 돌아오면(run 28 F7 · AI 가 빈 문장만 냄) 이전 초안을 지금 상태(밀린 값·거둔 뜻·결과 문구)로 다시 걸러 맞는 문장을 유지한다(옛 값 복원 0 · 고정 문장 0).
