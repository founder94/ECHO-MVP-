...
// 실행: node --experimental-strip-types --test spike/agent-v1-20260925/candidate-v3.3.test.mjs
import test from 'node:test';
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
const A = await import('./candidates/agent-v3.3.ts');
...
test('서비스 설명에 질문 개수 약속 0', () => {
  ...
  assert.equal(A.AGENT_VERSION, 'echo-agent-v3.3');
});
