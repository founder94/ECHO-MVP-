...
export function load() {
  ...
  return { failures: read('failures.json').failures, solutions: read('failed-solutions.json').solutions, graph: read('failure-graph.json'), families: read('families.json').families, types: read('types.json').types, golden: JSON.parse(readFileSync(GOLDEN, 'utf8')) };
}
...
export function renderLibrary(d) {
  ...
  const L = ['# ECHO Failure Library (v3 · 2026-09-25)', '',
    ...
    `- 합계 ${F.length}건 — 출처: ${by('origin')}`, `- 상태: ${by('status')}`, `- 방어 수준: ${by('defense_level')}`,
    ...
    '## 한눈에', '', '| ID | 날짜 | 출처 | Family | Type | 원인 Layer | 원인 확신 | 방어 수준 | 상태 |', '|---|---|---|---|---|---|---|---|---|',
    ...F.map((f) => `| ${f.id} | ${cell(f.date.split(',')[0].slice(0, 22))} | ${f.origin} | ${f.family} | ${cell(f.types.join(' · '))} | ${cell(f.layers.join(' · '))} | ${f.cause_confidence} | ${f.defense_level} | ${f.status} |`), ''];
  for (const f of F) {
    const row = (k, v) => (v ? `| ${k} | ${cell(v)} |` : null);
    L.push(`## ${f.id} ${f.title}`, '', '| 칸 | 내용 |', '|---|---|', ...[
      row('Family', `${f.family} — ${d.families.find((x) => x.id === f.family).name}`), row('발생 날짜', f.date), row('출처(증거)', `${f.origin} — ${f.origin_note}`),
      ...
      row('해결 후보', f.solution_candidates), row('실험 결과', f.experiment), row('부작용', f.side_effects), row('역검사 결과', f.counter_test), row('실AI 결과', f.real_ai),
...
export function renderSolutions(d) {
  return ['# Failed Solutions Archive (v2 · 2026-09-25)', '',
    ...
    ...d.solutions.map((s) => `| ${s.id} | ${cell(s.solution)} | ${cell(s.target)} | ${cell(s.result)} | ${cell(s.why_failed)} | ${cell(s.now)} | ${s.related.join(', ')} |`), ''].join('\n');
}
