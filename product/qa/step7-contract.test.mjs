...
// 2026-09-22 갱신: 첫 화면 계약은 그대로지만 구현 위치가 바뀌었다.
// 예전에는 index.html 의 echo-boot + /home 정적 import 로 막았다.
// 지금 진입 화면은 /(DoItEntry + DoItLandingPage)이고 둘 다 정적 import 다.
// 나머지 화면은 조각을 받는 동안 Suspense 대기 화면(RouteFallback)이 받는다.
test('first screen renders without a blank lazy-loading gap', async () => {
  const [html, routes, router, fallback] = await Promise.all([
    read('index.html'),
    read('src/router/config.tsx'),
    read('src/router/index.ts'),
    read('src/components/RouteFallback.tsx'),
  ]);

  // 1) 자바스크립트가 오기 전에도 흰 화면이 번쩍이지 않는다.
  assert.match(html, /body\s*\{[^}]*background:\s*#08070c/);
  assert.match(html, /<meta name="theme-color" content="#08070c" \/>/);

  // 2) 진입 화면은 지연 불러오기를 쓰지 않는다(조각을 기다리지 않는다).
  assert.match(routes, /^import DoItLandingPage from '@\/pages\/do-it\/landing\/page';$/m);
  assert.match(routes, /^import DoItEntry from '@\/pages\/do-it\/intro\/DoItEntry';$/m);
  assert.doesNotMatch(routes, /const DoItLandingPage = lazy/);
  assert.doesNotMatch(routes, /const DoItEntry = lazy/);

  // 3) 나머지 화면은 빈 화면(null)이 아니라 대기 화면이 받는다.
  assert.doesNotMatch(router, /fallback:\s*null/);
  assert.match(router, /fallback: createElement\(RouteFallback\)/);
  // 2026-09-23: 대기 화면은 심볼 3D 효과(SymbolLoader)에 문구를 넘기고, 읽어 주기 표시(role=status)는 그 부품이 단다.
  const loader = await read('src/components/SymbolLoader.tsx');
  assert.match(fallback, /<SymbolLoader [^>]*label="다음 화면을 여는 중이에요\."/);
  assert.match(loader, /role=\{label \? 'status' : undefined\}/);

  // 4) 가짜 진행률은 만들지 않는다(진행 막대·숫자 % 표시가 코드에 없어야 한다). 3D 효과 부품도 마찬가지.
  for (const code of [fallback, loader]) {
    assert.doesNotMatch(code, /role="progressbar"|aria-valuenow|<progress/i);
    assert.doesNotMatch(code, /\d+\s*%/);
  }

  // 5) 빨리 열리는 화면에서 글자가 번쩍이지 않도록 잠깐 기다렸다가 나타난다.
  assert.match(fallback, /QUIET_MS = 250/);
  assert.match(fallback, /if \(!visible\) return null;/);
});
