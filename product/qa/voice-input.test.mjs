...
test('못 쓰는 기기에서는 버튼을 숨길 수 있게 알려 준다 · 목소리를 서버로 보내는 코드 0', () => {
  ...
  assert.match(ui, /\{VOICE_CONVERSATION_ENABLED && voice\.supported && !\(voiceUi && !done\) && <button type="button" className=\{voice\.listening/, '2026-09-26 MVP: 음성 UI 는 VOICE_CONVERSATION_ENABLED(false) 뒤에 숨김');
