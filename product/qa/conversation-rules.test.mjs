...
test('서버 v13: 저장 경로 4곳(record_create/update, correct, self) 모두 규칙 차단을 거치고, 새 액션 2개가 등록돼 있다', () => {
  ...
  assert.equal((outside.match(/blockedContentReason\(/g) ?? []).length - 1, 9, 'RULES 정의 1 + 사용 9(create 2회·update·correct·self + v14.4 화면이 보낸 직전 질문 + v15 분류·직접 설명·카드 항목)');
...
test('v15.1 분류 규칙: 대화 자체를 가리키는 불만·지친 말은 계속 규칙으로 잡는다', () => {
  ...
  for (const t of ['지쳤어', '너무 피곤해', '아 귀찮아', '그만할래', '이제 그만 할래요', '답하기 싫어', '할말이없다 휴', '휴', '패스', '오늘은 여기까지']) assert.equal(rules.ruleKind(t), 'fatigue', t);
});
