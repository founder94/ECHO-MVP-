...
export const AGENT_VERSION = "echo-agent-v1.2";
...
- ask: 사용자가 너나 서비스에 질문했다 → reply 에서 먼저 제대로 답한다(서비스는 service_facts 안에서만, 모르면 모른다고). 그다음 next 는 current_question 과 같은 목적으로, 답을 못 받은 그 질문을 한 번 더 자연스럽게 묻는다(새 목적으로 넘어가지 않는다).
...
export interface Asked { type: "core" | "clarify"; purpose: string; text: string }
export interface TurnRec { n: number; ai: string | null; question_purpose: string | null; question_type: string | null; user: string; kind: string; saved?: boolean; extracted?: string[]; reply?: string; question?: string | null; decision?: string }
...
export interface TurnResponse { kind: string; reply: string; question: string | null; saved: boolean; extracted: { purpose: string; note: string }[]; finish: boolean; question_type: string | null; question_purpose: string | null }
...
export function applyTurn(st: AgentState, latest: string, out: Parsed, opts: { limitReached?: boolean } = {}): TurnResponse {
  ...
  if (SAVABLE.has(out.kind)) {
    for (const m of out.extracted) {
      ...
      const item: Item = { note: m.note, quote: m.quote, turn: turn.n, source: out.kind, status: "CONFIRMED" };
      st.slots[m.purpose].items.push(item); st.slots[m.purpose].status = "CONFIRMED"; kept.push({ purpose: m.purpose, note: m.note, turn: turn.n });
    }
  ...
  const pending = out.kind === "ask" && st.current && st.slots[st.current.purpose].status === "UNKNOWN" ? st.current : null;
  if (st.phase === "talk" && out.kind !== "stop" && !opts.limitReached) {
    ...
    if (pending) { if (n.question && (n.purpose === pending.purpose || !open.includes(n.purpose))) { pending.text = n.question; st.asked[st.asked.length - 1].text = n.question; } question = pending.text; decision = "keep_after_answer"; }
  ...
  return { kind: out.kind, reply, question, saved: turn.saved, extracted: kept.map(({ purpose, note }) => ({ purpose, note })), finish, question_type: question ? st.current!.type : null, question_purpose: question ? st.current!.purpose : null };
}
...
export function retryReason(st: AgentState, out: Parsed, left: string[], after: boolean): string {
  ...
  if (after || out.kind === "stop" || out.kind === "ask") return ""; // ask 는 지금 질문을 서버가 그대로 둔다
