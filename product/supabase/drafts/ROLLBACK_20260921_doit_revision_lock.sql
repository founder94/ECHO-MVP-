-- EMERGENCY REVIEW DRAFT ONLY. Routine rollback MUST keep the new revision safeguards.
-- This exceptional restore removes concurrency protection; requires separate explicit approval.
-- Do not run while the new follow-up feature is active.
BEGIN;
DO $rollback_preflight$
BEGIN
  IF md5(pg_get_functiondef('public.doit_apply_insight_generate(uuid,uuid,text,text,uuid,text,jsonb)'::regprocedure)) <> 'b79bf7c4b36017ff61e953c3ce8af6be' THEN
    RAISE EXCEPTION 'rollback refused: current function differs from reviewed v6 patch: doit_apply_insight_generate';
  END IF;
  IF md5(pg_get_functiondef('public.doit_apply_insight_self(uuid,uuid,text,text,uuid,text,text)'::regprocedure)) <> '413e6c5a2aff6d14f3b78e61539693a3' THEN
    RAISE EXCEPTION 'rollback refused: current function differs from reviewed v6 patch: doit_apply_insight_self';
  END IF;
  IF md5(pg_get_functiondef('public.doit_apply_insight_transition(uuid,uuid,text,text,uuid,integer,text,text)'::regprocedure)) <> '71088580893e3367519658f38b11ff37' THEN
    RAISE EXCEPTION 'rollback refused: current function differs from reviewed v6 patch: doit_apply_insight_transition';
  END IF;
  IF md5(pg_get_functiondef('public.doit_apply_record_create(uuid,uuid,text,text,text,text)'::regprocedure)) <> '913efc38f7ef74ce18e5695767b30353' THEN
    RAISE EXCEPTION 'rollback refused: current function differs from reviewed v6 patch: doit_apply_record_create';
  END IF;
  IF md5(pg_get_functiondef('public.doit_apply_record_create(uuid,uuid,text,text,text,text,text,text)'::regprocedure)) <> 'bd826007580ed7329d0be27230b131bf' THEN
    RAISE EXCEPTION 'rollback refused: current function differs from reviewed v6 patch: doit_apply_record_create';
  END IF;
  IF md5(pg_get_functiondef('public.doit_apply_record_update(uuid,uuid,text,text,uuid,integer,text,text)'::regprocedure)) <> 'b99010b877999769f00d4793ddeacb2f' THEN
    RAISE EXCEPTION 'rollback refused: current function differs from reviewed v6 patch: doit_apply_record_update';
  END IF;
END;
$rollback_preflight$;
