...
// 아이폰은 첫 소리를 사용자의 누름 안에서 시작해야 한다. 「말로」를 고른 그 누름에서 소리 길을 열어 둔다(같은 문서 안 화면 이동이라 유지된다).
export function unlockSpeech() { if (!canSpeak()) return; try { window.speechSynthesis.speak(new SpeechSynthesisUtterance(' ')); } catch { /* 글로 이어 간다 */ } }
